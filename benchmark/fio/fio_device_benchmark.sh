...
# Description: Nikoyo 测试单个磁盘的读写性能
# @Author: wangy
# @Date:   2016-12-21 16:41:42
# @Last Modified by:   wangy
# @Last Modified time: 2017-05-24 11:21:13
...
function usage() {
    cat << EOF
Usage:
    The tool can help you to test the benchmark of device(SATA SAS SSD) automatically.
    Please run such as './fio_device_benchmark.sh --storage_type iSCSI --filename /dev/sdb --hdparm 0 --rw randrw --rwmixread 70 --bs 4k --size 10G --runtime 30 --numjobs_array "1 2 4 8 16 24" --iodepth_array "1 2 4 8 16 32 64"'
...
# 设置Fio的filename, eg. /dev/sdb
while [[ -z "${filename}" ]]; do
    echo -n -e '\E[32;1m'"Please set the filename, eg. /dev/sdb : " ${tcreset}
    read filename
    echo ""
done
...
for numjobs in ${numjobs_array[@]}; do
    for iodepth in ${iodepth_array[@]}; do
        ...
        # 执行FIO测试命令
        if [[ "${rw}" == "rw" || "${rw}" == "randrw" ]]; then
            fio -name="${storage_type} ${bs} ${rw} test" -filename=${filename} --direct=1 --sync=1 -rw=${rw} -rwmixread=${rwmixread} -bs=${bs} -size=${size} -numjobs=${numjobs} -iodepth=${iodepth} -runtime=${runtime} --time_based -group_reporting >> ${log_file}
        else
            fio -name="${storage_type} ${bs} ${rw} test" -filename=${filename} --direct=1 --sync=1 -rw=${rw} -bs=${bs} -size=${size} -numjobs=${numjobs} -iodepth=${iodepth} -runtime=${runtime} --time_based -group_reporting >> ${log_file}
        fi
